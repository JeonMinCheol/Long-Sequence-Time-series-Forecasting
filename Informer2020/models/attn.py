...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        B, L, H, E = queries.shape # (batch, query_sequence, head_num, head_query_dimension)
        _, S, _, D = values.shape  # (batch, key_sequence, head_num, head_value_dimension)
        scale = self.scale or 1./sqrt(E)
        
        # 쿼리와 키 벡터 간의 유사도를 계산하여 어텐션 스코어 계산
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        queries = self.query_projection(queries).view(B, L, H, -1) # (batch, layer, head)
        keys = self.key_projection(keys).view(B, S, H, -1) # (batch, sequence, head)
